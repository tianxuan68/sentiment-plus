...
import { normalizePublicApiBaseUrl } from '/@/utils/thirdLoginUrl';
...
export function sysOAuth2Login(source) {
  let url = `${normalizePublicApiBaseUrl()}/sys/thirdLogin/oauth2/${source}/login`;
...
export function sysOAuth2Callback(code:string) {
  let url = `${normalizePublicApiBaseUrl()}/sys/thirdLogin/oauth2/dingding/login`;
