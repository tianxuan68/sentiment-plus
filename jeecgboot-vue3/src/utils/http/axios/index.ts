...
const defaultApiTimeout = Number(import.meta.env.VITE_GLOB_API_TIMEOUT) || 60 * 1000;
...
const transform: AxiosTransform = {
  ...
  beforeRequestHook: (config, options) => {
    ...
    formatDate && data && !isString(data) && !isFormData(data) && formatRequestDate(data);
    if (config.method?.toUpperCase() === RequestEnum.GET) {
      ...
      if (!isString(params)) {
        formatDate && !isFormData(params) && formatRequestDate(params);
